...
| `YOUTUBE_API_KEY` | Trailer/PV search, and the Music page's "New this season" / "Trending" YouTube feeds | Trailer sections and both YouTube music feeds show an honest "not configured" state instead of guessing a video. |
| `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET` | The Music page's live Spotify feeds (a real curator playlist's tracklist for "new this season" and "top anime songs"), plus real playlist creation on a user's own connected Spotify account | The Music page's Spotify sections show a clear "not configured" state (YouTube feeds and curated OP/ED credits still work); `/api/spotify/connect` redirects back with an honest `spotify_error=not_configured` instead of attempting the OAuth flow. Register `<NEXT_PUBLIC_APP_URL>/api/spotify/callback` as a Redirect URI in the Spotify Developer Dashboard for the app these keys belong to. |
| `AI_PROVIDER` + (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `AI_PLATFORM_API_KEY`) | AI-written Daily Brief executive summary | The summary falls back to a deterministic, fact-only template (no AI call) — see `src/lib/briefing/buildBriefing.ts`. `AI_PROVIDER` selects `"anthropic"` (default), `"openai"`, or `"goat-ai"`; **not currently set in production**, so the live app always runs the template path today. |
| `ANTHROPIC_MODEL`, `OPENAI_MODEL` | Model override | Defaults to `claude-sonnet-5` / `gpt-4o-mini`. |
| `AI_PLATFORM_API_KEY`, `AI_PLATFORM_BASE_URL` | `AI_PROVIDER=goat-ai` — a self-hosted, OpenAI-compatible platform reached via the official `openai` SDK with a custom `baseURL` (see `src/lib/ai/goat-ai.ts`) | Falls back to the template summary (same as above) unless both `AI_PROVIDER=goat-ai` and `AI_PLATFORM_API_KEY` are set. `AI_PLATFORM_BASE_URL` defaults to `https://api.gariyuuu.com/v1`. The model name (`"Yuu no Sekai"`) is fixed by the platform, not env-configurable. |
