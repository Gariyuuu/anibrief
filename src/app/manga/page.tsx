...
export default async function MangaBrowsePage({ searchParams }: { searchParams: Promise<{ q?: string; sort?: string }> }) {
  const { q, sort = "POPULARITY_DESC" } = await searchParams;

  const result = q
    ? await AniListProvider.searchMedia(q, "MANGA", { perPage: 30 })
    : await AniListProvider.browse({ type: "MANGA", sort: [sort as MediaSort], perPage: 30 });

  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-5">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Manga</h1>
        <p className="mt-1 text-sm text-muted">Browse and search AniList&apos;s full manga catalog.</p>
      </div>

      <form action="/manga" className="flex gap-2">
        <input
          name="q"
          defaultValue={q}
      ...
      <AnimeGrid items={result.items} emptyMessage={q ? `No manga matched "${q}".` : undefined} />
    </div>
  );
}
